...
let rec remove_not_feasible s clist ai acc extra =
	match clist with
	| [] -> acc @ extra
	| hd::tl -> if is_feasible s ai hd then
				let new_acc = acc @ [hd] in
				remove_not_feasible s tl ai new_acc extra
				else let new_extra = extra @ [hd] in 
				remove_not_feasible s tl ai acc new_extra
...
let determine_late_goal s ai =
	...
	let number_0 = get_number 0 board [] in
	...
	let costlist_0 = get_costs number_0 ai [] in
	...
	let orderedcostlist_0 = List.sort compare costlist_0 in
	...
	let correct0 = List.rev orderedcostlist_0 in
	...
	let goal0 = order_list number_0 correct0 ai [] in
	goal5 @ goal4 @ goal3 @ goal2 @ goal1 @ goal0
...
let take_three_gems_helper ai three =
	...
	let corrected = if List.length new_list >= 6 then
	sublist 0 3 new_list else new_list in
	{name = ai.name;
...
let has_color ai color = 
	match color with
	| Red -> if ai.gems_held.red > 0 then true else false
	| Green -> if ai.gems_held.green > 0 then true else false
	| Blue -> if ai.gems_held.blue > 0 then true else false
	| Black -> if ai.gems_held.black > 0 then true else false
	| White -> if ai.gems_held.white > 0 then true else false


let rec determine_discard s =
	let player_list = s.players in
	let num_players = List.length player_list in
	let corrected_num_players = num_players - 1 in
	let current_player = List.nth player_list corrected_num_players in
	...
	| Ai thelist -> let thesize = List.length thelist in
					...
					let thecolor = List.nth thelist correctedsize in
					if has_color current_player thecolor 
					then thecolor else
					let new_ai_list = sublist 0 correctedsize thelist in
					let new_player = 
						{
					 	name = current_player.name;
					 	gems_held = current_player.gems_held;
					 	discounts = current_player.discounts;
					 	points = current_player.points + 3;
					 	bought = current_player.bought;
